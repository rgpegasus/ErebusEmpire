  ...
  const fetchSeasons = async () => {
    try {
      ...
      if (currentSeason) {
        ...
        setSelectedSeason(firstSeason.url);
        const defaultSeasonId = firstSeason.url.split("/")[5];
        navigate(`/erebus-empire/anime/${animeId}/${defaultSeasonId}`, { replace: true });
      }
...
const handleEpisodeClick = async (episode) => {
  ...
  if (!selectedSeason.includes("scan")) {
    try {
  ...
  } else if (selectedSeason.includes("scan")) {
    try {
